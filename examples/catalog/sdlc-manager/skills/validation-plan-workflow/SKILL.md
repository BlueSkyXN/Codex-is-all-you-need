...
- The task is to design repo-specific testing layers, fixtures, coverage, or automation strategy. Use `test-strategy`.
- The task is to implement code, edit tests, or fix test failures. Use the relevant dev skill.
- The task is a release checklist after implementation. Use `release-check` when release execution is the main work.
...
For lane, ADS, lightweight ID, `local/sdlc`, delivery card, and midstream intake vocabulary, follow `../artifact-profile-router/references/sdlc-operating-model.md`.
...
- Do not replace `test-strategy`.
- Do not replace `dev_test_runner`.
- Do not replace `release-check`.
...
| Need | Next step |
|---|---|
| dev execution | `dev-handoff-planning` or `spec-driven-implementation` |
| repo-specific test strategy | `test-strategy` |
| test execution or failure diagnosis | `dev_test_runner` |
| traceability update | `requirements-traceability` |
| readiness decision | `sdlc-readiness-review` |
| release validation execution | `release-check` |
