...
| **集合** | 一个包内多条**同一意图族**的路线，配套文件成型 | `visual-brainstorming`、`hidden-watermark-risk-analysis` |
| **群** | 数十个独立 skill 的平级编队，可选 router 引导 | codex-next（约 60 叶 + `core-router` / `sdlc-router`） |
...
- 参考线约 30 个独立可触发 skill，且入口多样到需要引导，才值得建 router；本仓库实例 codex-next 约 60 叶配了两级 router。
...
**遗留形态**：历史包把说明 md 平铺在包根（早期个人仓库常见）不判不合格，但新建包不采用；迁移或大改时收进 `references/`。
...
| 版本 | 日期 | 说明 |
|---|---|---|
| 0.1 | 2026-07-16 | 首版草稿（未发布）：四档、目录白名单、内容模型、evals 章、群 ≥30 |
| 0.2 | 2026-07-17 | **收敛至实践与平台双基线**：删 `knowledge.map.yaml` 与包内快照约定；evals 形态自由化（不再规定 yaml 四件套字段）；examples 命名放开（good/bad 标注不锁前缀）；群 30 由硬门槛改量感参考，删计数口径与独立性测试清单；正文六节骨架降为建议、必须项收敛为四条；删「算子」术语；禁止清单 17 条并为 11 条；补生长谱系实例、平台扩展落点表、发货与敏感数据条款 |
