...
- The needed artifact is a measurable NFR matrix. Use `nfr-spec`.
- The needed artifact is a standalone HLD. Use `hld-workflow`.
- The needed artifact is a standalone LLD. Use `lld-workflow`.
- The needed artifact is a solution package that coordinates HLD/LLD/ADR/SPEC. Use `solution-spec-workflow`.
- The task is to create dev tasks. Use `dev-handoff-planning`.
...
| Slice ID | Source SRS IDs | NFR IDs | Acceptance IDs | Task candidate |
|---|---|---|---|---|
...
- Do not use a SPEC slice to bypass `dev-handoff-planning`.
...
| Need | Next skill |
|---|---|
| software requirement normalization | `srs-workflow` |
| measurable quality constraints | `nfr-spec` |
| standalone HLD | `hld-workflow` |
| standalone LLD | `lld-workflow` |
| domain ownership or boundary map | `domain-boundary-modeling` |
| modular monolith directory/dependency structure | `modular-monolith-architecture` |
