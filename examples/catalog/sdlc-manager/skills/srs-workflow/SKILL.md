...
- There is no approved or clearly marked requirement source; use `requirements-workflow` or `prd-workflow` first.
- The request is to decide final architecture, implementation libraries, database technology, or repository refactor strategy. Use `solution-spec-workflow` for structured solution material and let dev handle execution.
...
| Prefix | Meaning |
|---|---|
| `SRS-FR` | Functional requirement |
| `SRS-IR` | Interface requirement |
| `SRS-DR` | Data requirement |
| `SRS-PR` | Permission requirement |
| `SRS-OR` | Operational requirement |
| `SRS-CR` | Compliance requirement |
| `SRS-AC` | Acceptance criterion |
| `SRS-RISK` | Risk or constraint |
| `SRS-Q` | Open question |
...
- Link each SRS ID to at least one source input when possible.
...
| NFR reference | measurable quality constraint managed by `nfr-spec` |

Keep NFR detail in `nfr-spec` when the NFR is substantial. The SRS should reference NFR IDs rather than duplicating the full matrix.
...
For substantial data design, hand off to `spec-slice-writer` for a Data SPEC.
...
For substantial authorization behavior, hand off to `spec-slice-writer` for a Permission SPEC.
...
Do not hide quality constraints inside vague prose. Use `nfr-spec` when measurements, thresholds, validation methods, or release gates are needed.

### 11. Produce a traceability seed

Create an initial mapping table:

| Source | SRS ID | Requirement summary | Acceptance ID | SPEC needed | Task candidate | Test candidate |
|---|---|---|---|---|---|---|

This is not the final RTM. It is the seed for `requirements-traceability`.
...
- Do not replace `nfr-spec`, `spec-slice-writer`, `solution-spec-workflow`, or `dev-handoff-planning` when those specialized outputs are needed.
...
| Need | Next skill |
|---|---|
| measurable quality constraints | `nfr-spec` |
| UI/API/Data/Admin/Permission/Directory specs | `spec-slice-writer` |
| HLD/LLD-oriented solution material | `solution-spec-workflow` |
| implementation task package | `dev-handoff-planning` |
