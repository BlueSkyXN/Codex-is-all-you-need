...
如果用户要把能力地图转成正式需求或规格，按材料类型转交 `sdlc-requirements-workflow`、`sdlc-prd-workflow`、`sdlc-srs-workflow`、`sdlc-spec-slice-writer` 或 `sdlc-requirements-traceability`。如果用户要拆研发任务，转交 `sdlc-dev-handoff-planning`。如果用户要打分、写汇报话术或做绩效评价，应在能力地图完成后另起下游步骤。
