...
|---|---|---|
| explicit Codex Next SDLC coordination | User-invoked control-plane planning and recommendation before a separately invoked authoring or review workflow | `sdlc-manager`, `sdlc-router` |
...
`sdlc-manager` is the explicit bundled SDLC coordination entrypoint. It plans
the Architecture-first sequence and recommends one separately invoked next
workflow; it does not author, invoke, or imitate requirements,
...
`sdlc-manager` 是插件内需要显式调用的 SDLC 协调入口。它规划架构先行的执行顺序，
