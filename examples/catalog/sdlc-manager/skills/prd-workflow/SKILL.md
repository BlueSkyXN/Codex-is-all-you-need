...
- Use `srs-workflow` to convert product behavior into software requirements.
- Use `nfr-spec` for quality, security, privacy, performance, availability, accessibility, compatibility, observability, or compliance.
- Use `spec-slice-writer` for UI/API/Data/Admin/Permission/Directory specs.
- Use `dev-handoff-planning` only after SRS/SPEC inputs are sufficient for implementation tasks.
