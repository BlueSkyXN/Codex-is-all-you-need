...
Do not use this skill for simple read-only explanation when no delivery decision is needed. Use `direct-read` / `repo-onboarding` style exploration instead.
...
- Read-only repo exploration: `repo-onboarding`
- Bugfix or urgent repair: `bugfix`
- Direct implementation from clear input: `spec-driven-implementation`
- Requirements package or Product Pack: `requirements-workflow`, `prd-workflow`, `brd-workflow`, `urs-workflow`
- Software requirements or NFR: `srs-workflow`, `nfr-spec`
- Architecture or domain impact: `hld-workflow`, `domain-boundary-modeling`, `architecture-decision-record`, `modular-monolith-architecture`
- Detailed design or implementation-facing slices: `lld-workflow`, `solution-spec-workflow`, `spec-slice-writer`
- Handoff: `dev-handoff-planning`
- Validation plan: `validation-plan-workflow`
- Traceability or change control: `requirements-traceability`, `change-control`
- Readiness check: `sdlc-readiness-review`
- Refactor or migration planning: `refactor-plan`, `migration-plan`
