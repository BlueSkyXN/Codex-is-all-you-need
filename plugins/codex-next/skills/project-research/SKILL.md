...
`project-research` 在 `sdlc-manager` 中专注于项目功能能力地图和证据入口，不再承载旧产品工程研究包。

当研究用于重建、替换或迁移时，按 `../artifact-profile-router/references/sdlc-operating-model.md` 把能力地图作为范围决策表的证据输入。不要一次性写完整未来 PRD/SRS/RTM；只把旧能力标成保留、舍弃、改造或存疑，并路由到首波需求包、规格或 handoff。
...
如果用户要把能力地图转成正式需求或规格，按材料类型转交 `requirements-workflow`、`prd-workflow`、`srs-workflow`、`spec-slice-writer` 或 `requirements-traceability`。如果用户要拆研发任务，转交 `dev-handoff-planning`。如果用户要打分、写汇报话术或做绩效评价，应在能力地图完成后另起下游步骤。
...
14. 默认只写这 3 个产物：
    ...
    - `项目功能能力地图.md`
    - `项目能力表.csv`
15. 如果是重建、替换或迁移场景，追加一张范围决策表草案或明确说明为什么暂不生成：
   - 保留：旧能力进入新系统。
   - 舍弃：旧能力明确不进入本轮或新系统。
   - 改造：旧能力保留目标但实现、流程、口径或体验变化。
   - 存疑：需要用户确认，使用 `Q-001` 记录。
16. 如果用户要求 XLSX / Excel / 工作簿 / 表格阅读版，用同一份 CSV 数据额外生成 `项目能力表.xlsx`，不要让 XLSX 与 CSV 内容漂移。
17. 最后输出简短交付摘要：产物路径、一级/二级/三级数量、主要证据缺口、3 句可用于汇报的中文表达；如果生成 XLSX，也说明工作簿的工作表结构。
...
| 后续需求 | 推荐 skill |
|---|---|
| 业务需求、用户需求或产品范围 | `requirements-workflow`, `prd-workflow` |
| 软件行为要求、验收条件、字段/权限/状态 | `srs-workflow` |
| UI/API/Data/Admin/Permission/Directory 规格切片 | `spec-slice-writer` |
| 质量、安全、性能、可用性、隐私或合规约束 | `nfr-spec` |
| 能力、需求、规格、任务、测试之间的追踪 | `requirements-traceability` |
| 已具备规格后的研发任务包 | `dev-handoff-planning` |
| 旧系统重建、替换或迁移范围收口 | 范围决策表 -> `requirements-workflow` 或首波 `dev-handoff-planning` |
