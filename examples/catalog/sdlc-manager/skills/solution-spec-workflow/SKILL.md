...
A solution package connects SRS/NFR/HLD/LLD/ADR/Domain Boundary Map/SPEC materials to an implementation approach. This workflow coordinates and summarizes those artifacts; it should not replace the dedicated `hld-workflow`, `lld-workflow`, `domain-boundary-modeling`, `modular-monolith-architecture`, or `architecture-decision-record` workflows when those artifacts need to be authored in full.
...
- The user asks only to author a standalone HLD. Use `hld-workflow`.
- The user asks only to author a standalone LLD. Use `lld-workflow`.
- The user asks only to model domains, bounded contexts, or data ownership. Use `domain-boundary-modeling`.
- The user asks only to record an architecture decision. Use `architecture-decision-record`.
- Required behavior is not specified yet. Use `srs-workflow` or `spec-slice-writer` first.
- The task is only to generate dev task cards. Use `dev-handoff-planning`.
...
| Level | Use for |
|---|---|
| HLD coordination | summarize or link system boundaries, components, flows, integrations, data movement, major dependencies from `hld-workflow` |
| LLD coordination | summarize or link module responsibilities, interfaces, algorithms, state transitions, error handling, migration steps from `lld-workflow` |
| Domain coordination | summarize or link domain ownership, data ownership, allowed dependencies, and forbidden dependencies from `domain-boundary-modeling` |
| ADR coordination | summarize or link architecture decisions, alternatives, consequences, and review conditions from `architecture-decision-record` |
...
| Solution non-goal | redesign unrelated service, migrate all legacy state, replace auth system |
...
If this information is substantial or contested, route to `hld-workflow` and link the resulting HLD here.
...
If the decision is long-lived or affects HLD, LLD, SPEC, RTM, or future implementation, route it to `architecture-decision-record` and link the ADR here.
...
Use `spec-slice-writer` for detailed Data SPEC when needed.
...
For modular-monolith directory and dependency rules, use `modular-monolith-architecture` or Directory SPEC, then summarize the accepted constraints here.
...
If the notes become detailed module design, route to `lld-workflow` and link the LLD here.
...
Hand off to `dev-handoff-planning` for task card creation.
...
- Dev handoff notes are concrete enough for `dev-handoff-planning`.
...
| Need | Next skill |
|---|---|
| missing software requirements | `srs-workflow` |
| missing measurable quality requirements | `nfr-spec` |
| missing HLD | `hld-workflow` |
| missing LLD | `lld-workflow` |
| missing domain boundary or ownership map | `domain-boundary-modeling` |
| missing modular monolith structure | `modular-monolith-architecture` |
| missing architecture decision record | `architecture-decision-record` |
| missing UI/API/Data/Admin/Permission slices | `spec-slice-writer` |
| executable implementation task package | `dev-handoff-planning` |
| requirement-to-task/test traceability | `requirements-traceability` |
