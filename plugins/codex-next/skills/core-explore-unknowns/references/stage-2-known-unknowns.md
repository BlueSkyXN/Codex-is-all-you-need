...
3. Close every question one of three ways:
   ...
   - **Answered by the territory** — go read the code instead of asking, then
     show the user the question and the found answer. A question closed
     off-screen isn't closed.
...
- **Brainstorm the intervention** — when the question is "which solution?":
  search the codebase first, then plot interventions from ship-this-afternoon
  to quarter-long bet, each grounded in what actually exists.
- **Point at a reference** — when an existing implementation encodes the
  ...
  it does with excerpts, how each behavior maps to the target stack.
