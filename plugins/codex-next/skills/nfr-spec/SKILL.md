...
- The user asks for architecture decisions without a requirement source. Use `hld-workflow`, `architecture-decision-record`, or `solution-spec-workflow` after NFRs are identified.
- The quality requirement cannot be evaluated yet because the product behavior is still undefined. Use `srs-workflow` or `prd-workflow` first.
...
If the NFR implies a long-lived architecture decision or meaningful trade-off, create or request an ADR via `architecture-decision-record`.
...
- Do not replace `solution-spec-workflow` or `dev-handoff-planning`.
...
| Need | Next skill |
|---|---|
| software requirement mapping | `srs-workflow` |
| UI/API/Data/Admin/Permission/Directory slices | `spec-slice-writer` |
| HLD | `hld-workflow` |
| LLD | `lld-workflow` |
