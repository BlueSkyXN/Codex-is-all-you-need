...
本仓库用于发布可公开的 Codex preset 系统：脱敏后的 agent / skill catalog
示例、Codex Next 插件包、架构与迁移文档、只读 dashboard，以及 legacy/local-dev
repo-local entrypoint 管理脚本。

- `README.md` 与 `README_CN.md`：英文和中文项目入口说明。
- `docs/`：架构、agent 设计、skill 设计、迁移策略、公开/私有分层说明。
- `plugins/`：可安装的 Codex plugin packages，其中 `plugins/codex-next` 是公开共享
  skills 的 canonical packaged surface。
- `.agents/plugins/marketplace.json`：暴露仓库内插件的 marketplace metadata，
  不是过时 runtime state。
...
- `examples/runtime/` 与 `examples/suites/`：runtime 和 legacy/local-dev suite 布局示例。
- `dashboard/`：Python 静态 dashboard 生成器、HTML 模板和示例配置。
- `scripts/`：legacy/local-dev repo-local `.codex` entrypoint symlink 管理脚本。
...
Agent 文件使用 snake_case，例如 `dev_python_engineer.toml`。Skill 目录使用带领域前缀的 kebab-case，例如 `dev-python-quality/`，入口文件固定为 `SKILL.md`。
...
不要把私有 skills、runtime 路径、生成的扫描结果、机器特定 symlink 状态，以及真实的
`~/.codex/dashboard/config.toml` 值提交到仓库。Dashboard 应保持只读；生产态共享
skills 优先通过已安装的 Codex Next 插件提供，legacy/local-dev suite symlink 只用于兼容、
实验或 custom agent 暴露。除非任务明确要求，否则不要创建、删除或重写 `.codex`、
`.agents`、suite symlink 或 source catalog 文件；不要把
`.agents/plugins/marketplace.json` 当作可清理的 runtime state。
